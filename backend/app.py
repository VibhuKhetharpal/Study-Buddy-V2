...
def log_activity():
    ...
    app_window = f"{app_name} {window_title}"
...
def label():
    ...
    update_user_label(log_id, user_label.lower())
...
def bulk_label():
    ...
    bulk_label_window(session_id, window_title, user_label.lower())
    return jsonify({"success": True}), 200
...
def session_summary(session_id):
    ...
    if not api_key:
        return jsonify({"error": "GEMINI_API_KEY not found in environment"}), 400
